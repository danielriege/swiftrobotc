...
private:
    ...
    uint16_t port;
    std::function<void(swiftrobot_packet_type_t type, char* data, size_t size)> receivedPacketCallback;
...
public:
    ...
    void send2Packet(swiftrobot_packet_type_t type, int deviceID, char* data1, size_t size1, char* data2, size_t size2);
    void send2PacketToAll(swiftrobot_packet_type_t type, char* data1, size_t size1, char* data2, size_t size2);
    void registerReceiveCallback(std::function<void(swiftrobot_packet_type_t type, char* data, size_t size)> callback);
